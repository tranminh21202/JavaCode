...
class SXXangDau {
    ...
    public SXXangDau(String ma, long sl) {
        this.ma = ma;
        this.sl = sl;
    }
...
public class SapXepXangDau {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<SXXangDau> list = new ArrayList<>();
        int t = Integer.parseInt(sc.nextLine());
        for (int i = 1; i <= t; i++) {
            String ma = sc.next();
            long sl = Long.parseLong(sc.next());
            SXXangDau a = new SXXangDau(ma, sl);
            list.add(a);
        }
        Collections.sort(list, Comparator.comparingLong(SXXangDau::getTien).reversed());
        for (SXXangDau x : list) {
